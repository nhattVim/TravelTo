  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-[#d3efe4] bg-white shadow-[0_14px_30px_rgba(14,95,68,0.08)] transition-transform duration-300 hover:-translate-y-1">
      <div className="relative h-52 overflow-hidden">
        ...
        />
        <div className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-[#0a6f50]">
          {tour.provinceName}
        </div>
      </div>
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="flex items-center justify-between">
          <p className="text-lg font-bold text-[#0a7d59]">{formatCurrencyVnd(tour.price)}</p>
          <Link
            href={`/tours/${tour.id}`}
            className="rounded-full bg-[#0a7d59] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#085a41]"
          >
            Xem tour
          </Link>
        </div>
